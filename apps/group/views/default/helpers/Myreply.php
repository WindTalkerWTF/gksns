 ...
 */
class Group_View_Helper_Myreply
{
    /**
     ...
     * 
     */
    public function myreply($mark,$canPost=1){
    	...
    	//是否是小组成员
    	$arcidTmp = explode("#",$mark);
    	$arcId = (int) $arcidTmp[1];
    	$checkTmp = Group::dao()->getArc()->get(array("id"=>$arcId));
    	$gid = $checkTmp['group_id'];
    	$this->view->isMember = 0;
    	$members = Group::dao()->getUser()->get(array("uid"=>$this->view->uid,"group_id"=>$gid));
    	if($members){
    		$this->view->isMember = 1;
    	}
